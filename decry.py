...
def decrypt():
    ...
    # Algorithm to decrypt the info from the image
    ...
    for index_i, i in enumerate(img):
        ...
        for index_j, j in enumerate(i):
            if((index_j) % 3 == 2):
                # first pixel
                info.append(bin(j[0])[-1])
                # second pixel
                info.append(bin(j[1])[-1])
                # third pixel
                if(bin(j[2])[-1] == '1'):
                    stop = True
                    break
                ...
                info.append(bin(j[1])[-1])
                # third pixel
                info.append(bin(j[2])[-1])
        if(stop):
            break

    message = []
    # join all the bits to form letters (ASCII Representation)
    for i in range(int((len(info)+1)/8)):
        message.append(info[i*8:(i*8+8)])
